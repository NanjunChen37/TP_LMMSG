...
def run(psi, input_fasta, target_folder, db, nr = None):
    ...
    # blosum
    rest_names = check(names, pssm_folder)
    message = f'length of sequence not find in nr90 is {len(rest_names)}'
    print(message)
...
def gen_pssm_by_blast(psi, src, pssm_folder, xml_folder, db):
    ...
    os.system(cmd)
    # os.popen(cmd)
    # psiblast -query data/test_data/neg_demo/pssm/tmp/sp_Q91FT2_240R_IIV6.fasta -db utils/psiblast/nrdb90/nrdb90 -out data/test_data/neg_demo/pssm/xml/sp_Q91FT2_240R_IIV6.xml -evalue 0.01 -num_iterations 3 -num_threads 6 -out_ascii_pssm data/test_data/neg_demo/pssm/output/sp_Q91FT2_240R_IIV6.pssm -outfmt 5
